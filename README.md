...
- Принцип работы:
  - Получить gcode из midi с сайта-конвертера
  - Загрузка gcode(.nc) файла в программу
  - Настройка параметров gcode'а(тип прошивки, ускорение, отключение моторов, пауза перед и после музыки)
  - Конвертирование
  - Экспорт gcode'а(копирование, сохранение как .gcode файл)
  - Отправка на ЧПУ с CoreXY механикой
- gcode на выходе из программы:
  - Автопарковка(G28)
...
- Необходимо внести информацию о принтере
  ...
  - Каналы: рекомендуется оставить все включёнными. Если принтер издаёт непонятные звуки(музыка не похожа на оригинал), то необходимо выключить некоторые каналы, которые мешают более важным каналам
...
- Кнопка **Save file** скачает gcode в файле с расширением .nc
...
- Синтаксис должен быть как у конвертера выше. В начале файла параметры принтера; перед началом основного gcode'а находятся команды G21, G90, G94, G00/G01; в конце команда M02
...
- Если выставить слишком большое значение, принтер может начать издавать щёлкающие звуки(из-за слишком быстрого разгона или из-за пропуска шагов). Также частоты звуков могут немного изменяться из-за работы алгоритма INPUT SHAPING(асли он есть)
...
#### Выключение моторов(M84)
![m84](doc/m84.jpg)
- Выключение моторов после музыки влияет на нагрев моторов
- Если выключать моторы, то они и драйвера смогут немного охладиться
...
- В конце, если моторы выключаются(M84), они издают щелчок(стук)
...
- Моторы работают в режиме StealthChop, либо включена интерполяция, либо всё вместе
  - Режим работы драйверов настраивается, если они подключены по uart. Надо вместо режима StealthChop поставить SpreadCycle. Изменить можно в коде прошивки/cfg файле(зависит от прошивки)
  - Интерполяция настраивется также, как и режим работы. Интерполяцию необходимо отключить
...
- Скорее всего Вы неправильно указали количество шагов на миллиметр на сайте конвертера
  - На данный параметр влияют шкивы на моторах, моторы(шаги на оборот), редуктор(если есть) между мотором и шкивом/валом
  - Шаги_на_мм **=** (шаги_на_оборот **x** микрошаг) **/** мм_на_оборот **x** соотношение_редуктора(если есть)
  - Рекомендуемый микрошаг для мотора 200 шагов на оборот и 40 мм на оборот - 16 микрошагов на шаг
